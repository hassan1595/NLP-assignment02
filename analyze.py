...
class Analyze:
    ...
    def general_insights(self):
        """
        Generates and saves general insights about the dataset, including:
        - Number of samples
        - Covariance matrix of word counts
        - Scatter plot of word counts in English and Italian
        """
    ...
    def average_n_words_plot(self):
        ...
        avg_en = np.mean([len(en_doc.split()) for en_doc in en_data])
        avg_it = np.mean([len(it_doc.split()) for it_doc in it_data])
    ...
    def get_unique_words(self, text_list):
        """
        Retrieves a set of unique words from a list of texts.
    ...
    def average_n_sentence_plot(self):
        ...
        """
        en_data, it_data = self.dp.get_raw_data()
        avg_en = np.mean([len(self.split_into_sentences(en_doc)) for en_doc in en_data])
        avg_it = np.mean([len(self.split_into_sentences(it_doc)) for it_doc in it_data])

        self.plot_bar(["English", "Italian"], [avg_en, avg_it], "Languages", "Average Number of Sentences", (avg_en + avg_it) / 2, "avg_num_sents.png")

    def create_plots(self):
        """
        Creates all plots by calling the relevant methods.
        """
